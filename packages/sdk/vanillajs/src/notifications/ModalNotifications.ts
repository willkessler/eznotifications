...
export class ModalNotification {
  ...
  async show(notification:SDKNotification):Promise<boolean> {
    ...
    try {
      this.modalOn = true;
      console.log(`Inside ModalNotification constructor we have configuration: ${JSON.stringify(configuration,null,2)}`);
      let showConfirm = true;
      let showDismiss = true;
      const { modal } = configuration;
      if (modal?.show) {
      ...
      const swalOutcome:SweetAlertResult = await Swal.fire({
        ...
        customClass: (configuration.modal?.useCustomClasses ? customClassObject : {}),
