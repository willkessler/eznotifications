...
export class InlineNotification {
  ...
  private TINAD_INNER_CONTAINER_CLASSNAME = 'tinad-inner';
  private TINAD_NOTIFICATION_CONTENT_CLASSNAME = 'tinad-content';
  ...
  constructor(configuration:SDKConfiguration ) {
    ...
    console.log(`constructor: ${this.configuration.inline.targetClassname}`);
    this.buildAllContainers();

  }

  private buildAllContainers() {
    // if user passed a target class then we should make sure that target exists, and then create
    // tinad containers before, inside, or after that target.
    this.targetElementClassname = 
      (this.configuration.inline.targetClassname !== null ? 
       this.configuration.inline.targetClassname : 
       this.TINAD_CONTAINER_DEFAULT_CLASSNAME);
    const targetContainers = Array.from(document.querySelectorAll('.' + this.targetElementClassname));
    console.log('buildContainers() running, targetContainers:', targetContainers);
    if (targetContainers.length == 0) {
      ...
      if (this.targetElementClassname !== this.TINAD_CONTAINER_DEFAULT_CLASSNAME &&
          this.configuration.inline.customControlClasses) {
        console.log('Setting up custom control classes for non tinad container');
        ...
        console.log('Setting up default classes');
        for (const container of targetContainers) {
          const notificationElement = document.createElement('div');
          notificationElement.className = this.TINAD_INNER_CONTAINER_CLASSNAME;
          notificationElement.style.display = 'none';

          const contentDiv = document.createElement('div');
          contentDiv.className = this.TINAD_NOTIFICATION_CONTENT_CLASSNAME;
      ...
      this.tinadNotificationContentElements = Array.from(document.querySelectorAll('.' + this.TINAD_NOTIFICATION_CONTENT_CLASSNAME));
    }
    // Any div element inside the tinad-containers is for holding the content.
    this.inlineNotifOn = false;
    console.log('Tinad: Container builds done');
  }
